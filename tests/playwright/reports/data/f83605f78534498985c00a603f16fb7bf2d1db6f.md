...
- Name: inventory.spec.mjs >> 🔧 Mantenimiento - /inventario/mantenimiento >> 2.2 Tres tabs están presentes (Categorías, Marcas, Unidades)
- Location: tests\playwright\inventory.spec.mjs:322:5
...
> 107 |     await page.fill('input[name="email"]', CREDENTIALS.email);
  ...
  200 |                 const options = await sel.locator('option').allTextContents();
  201 |                 console.log(`  ℹ️  Select ${i} opciones: ${options.join(', ')}`);
  202 | 
  203 |                 if (options.some(o => o.includes('ENTRADA') || o.includes('Entrada'))) {
  204 |                     await sel.selectOption({ label: options.find(o => o.includes('ENTRADA') || o.includes('Entrada')) || '' });
  205 |                     await page.waitForTimeout(500);
  206 |                     await screenshot(page, '03-kardex-filter-entrada');
  207 |                     break;
```